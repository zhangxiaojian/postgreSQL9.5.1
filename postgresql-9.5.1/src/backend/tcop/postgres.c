...
exec_simple_query(const char *query_string)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		/* clear and reset isolation level <add by zhangxiaojian> 
		 * we don't need to reset isolevel to default after transaction commite.
   		 * the StartTransaction will do it for us.	
		 */
		MyPgXact->vacuumFlags &= PROC_ISOLATION_CLEAR;
		switch(XactIsoLevel){
			case 0:
			case 1:
				MyPgXact->vacuumFlags |= PROC_READ_COMMITTED;
				break;
			case 2:
				MyPgXact->vacuumFlags |= PROC_REPEATABLE_READ;
				break;
			case 3:
				MyPgXact->vacuumFlags |= PROC_SERIALIZABLE;
				break;
		}
