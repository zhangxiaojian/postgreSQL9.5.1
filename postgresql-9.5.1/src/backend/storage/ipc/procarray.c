...
/**
 * return the oldest xmin which trasaction's isolation level > readcommitted
 * <add by zhangxiaojian>
 */
TransactionId
GetOldestXminVacuumOptimized(Relation rel, bool ignoreVacuum)
{
	ProcArrayStruct *arrayP = procArray;
	TransactionId result;
	int			index;
	...
	for (index = 0; index < arrayP->numProcs; index++)
	{
			...
			proc->databaseId == 0)		/* always include WalSender */
		{
			...
			int	isolevel = XACT_READ_UNCOMMITTED;
			if(pgxact->vacuumFlags & PROC_SERIALIZABLE)
				isolevel = XACT_SERIALIZABLE;
			else if(pgxact->vacuumFlags & PROC_REPEATABLE_READ)
				isolevel = XACT_REPEATABLE_READ;
			else if(pgxact->vacuumFlags & PROC_READ_COMMITTED)
				isolevel = XACT_READ_COMMITTED;

			/* First consider the transaction's own Xid, if any */
			if (TransactionIdIsNormal(xid) &&
				TransactionIdPrecedes(xid, result) && isolevel > XACT_READ_COMMITTED)
				result = xid;
